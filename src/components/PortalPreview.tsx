...
import type { AssetStatus, ClipLink } from '../types';
...
function getMissingAssetCount(guest: PublicGuestData): number {
  return [
    guest.bioStatus,
    guest.headshotStatus,
    guest.socialHandleStatus,
    guest.releaseFormStatus,
  ].filter((status) => status === 'needed' || status === 'requested').length;
}

function getShareProgress(guest: PublicGuestData): string {
  const checks = [
...
export default function PortalPreview({ guest, voice = 'guest' }: PortalPreviewProps) {
  const score = getReadinessScore(guest as never);
  const signals = getReadinessSignals(guest as never);
  const missing = getMissingAssetCount(guest);
  const shareLabel = getShareProgress(guest);
  const liveLinks = [guest.youtubeLink, guest.spotifyLink, guest.appleLink, guest.episodeLink].filter(Boolean).length;

  const sentence =
    voice === 'guest'
      ? getReadinessSentence(score)
      : getReadinessSentence(score).replace("You're", `${firstName(guest.name)} is`);
    ...
    <div className="portal-page">
          ...
          <div className="portal-pass__inner">
            <div className="portal-pass__ring-block">
              ...
              <div className="portal-pass__ring-stats">
                ...
                <span><strong>{shareLabel}</strong> shared</span>
      ...
      <section className="portal-section">
        ...
        {(guest.releaseFormLink || guest.pressKitLink) && (
          <div className="portal-chips" style={{ marginTop: 6 }}>
